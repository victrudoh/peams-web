...
const AllCategories = () => {
  ...
  const gotoCategory = async (id) => {
    ...
    navigate(`/admin/categories/one?id=${id}`);
  };
  ...
  const onSearchCangeHandler = async (e) => {
    try {
      e.preventDefault();
      const filteredUser = allCategories.filter((item) =>
  ...
  // populate filtered with allCategories on page load
  useEffect(() => {
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 p-4">
        {filtered ? (
          ...
          <>
            <span>No categories, add some?</span>
          </>
        )}
      </div>
    </>
