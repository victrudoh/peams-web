...
const AddCategory = () => {
  const { loading, setLoading, getAllCategories, setShowAddCategory } =
  ...
  const addProductHandler = async (e) => {
    ...
    try {
      const response = await axios.post(
        `https://peams-api.onrender.com/api/categories/add`,
        categoryDetails,
        {
      ...
      console.log(
        "🚀 ~ file: AddCategory.jsx:28 ~ addProductHandler ~ response:",
        response
      );
      ...
      <div className="absolute bg-primary-color/40 top-0 left-0 z-10 min-h-screen min-w-full flex items-center justify-center">
        ...
        >
          <div className="w-full flex items-center justify-between mb-4">
            <span className="font-bold text-black-ish">Add Category</span>
            <span
              className="font-bold text-error-color text-2xl font-sans cursor-pointer hover:text-xl"
